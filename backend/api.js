...
app.post("/forgot-password", async (req, res) => {
    ...
    try {
        ...
        if (hashedPassword) {
            ...
            const resetLink = `https://jobsearch-n4zw.onrender.com/reset-password-page?reset-token=${resetToken}`;
...
app.get(/^\/(?!register$|login$|upload-pdf$|user-profile$|update-username$|update-password$|delete-account$|forgot-password$|delete-reset-token$|validate-token$|validate-reset-token$|get-jobs$|get-logo$).*/, (req, res) => {
  res.sendFile(path.join(__dirname, 'build', 'index.html'));
});
